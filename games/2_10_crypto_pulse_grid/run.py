"""Driver script for Crypto Pulse Grid (2_10).
...
Twenty modes call_140 .. call_10000 (the 1.4x-100x multiplier ladder), each its own
cost-1.0 win/lose bet mode. Each mode's num_sims = the exact denominator b of its win
probability a/b, so published book counts equal the win chance; realised RTP is
(a/b)*multiplier, pinned into [96.00%, 96.70%]. num_threads = 1 and
batching_size >= every mode's num_sims (max 135) so each mode is one exact batch.
