#!/usr/bin/env python3
"""
Build the frontend-demo data bundle for Chicken Run (2_8) from the REAL generated
...
Reads the game's published library:
    library/publish_files/index.json                -> the 72 modes + their LUTs
...
multiplier or 0), so each collapses to two outcomes. Emits ONE file the demo fetches:
    frontend_demo/chicken_run_rgs.json  { game_id, difficulties, lanes, rtp,
      ladders:{easy:[..24],medium:[..],hard:[..]}, modes:{"easy_1":{...}, ...} }
...
OUT_FILE = os.path.join(HERE, "chicken_run_rgs.json")
...
def main():
    ...
    game_id = cfg.get("gameID", "2_8_chicken_run")
    ...
    for d in DIFFICULTIES:
        lad = ladders[d]
        print(f"  {d:7s} lanes={len(lad)} max={lad[-1]}x  lane1={lad[0]}x")
    print(f"  RTP range: {min(rtps)}..{max(rtps)}")
    if max(rtps) > ACP_CEIL + 1e-9:
        print(f"  NOTICE: RTP {max(rtps):.4f} exceeds the ACP ceiling {ACP_CEIL}.")
