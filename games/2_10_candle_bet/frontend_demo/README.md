# Candle Bet (2_10) — frontend demo

A self-contained browser mockup of the UP/DOWN mechanic that **replays the published
math**. Pick UP or DOWN; a BTC/USD chart animates ~5–8s and finishes above/below the
...
exactly two payouts `{0, win}`, and emits **`candle_bet_rgs.json`**:

```json
{ "game_id":"2_10_candle_bet", "rtp":0.9667, "multiplier":1.9, "winChance":0.508772,
...
demo's realised odds match the certified LUT. UP and DOWN share the single `base`
mode — the chart direction is derived client-side: `endsHigh = (pickedHigh === isWin)`.

**Rebuild the bundle after any math rebuild:**

```sh
PYTHONPATH="$(pwd)" ./env/bin/python games/2_10_candle_bet/frontend_demo/build_demo_data.py
```
