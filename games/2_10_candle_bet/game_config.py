"""
Candle Bet (2_10) — game configuration.

A bet on the **NEXT CANDLE** of a candlestick chart. The player picks a candle SIZE
(ANY / BIG / HUGE) and calls **UP or DOWN**; the next candle forms and settles on
its close vs its open. The round **wins** the offered multiplier if the certified
verdict lands the player's way, otherwise pays 0. Like the dice (`2_4`), limbo
(`2_5`) and chicken (`2_7`/`2_8`) games this is a **direct-probability** game: no
board, no reels, no free spins, Rust optimiser disabled. The odds come straight
from the distribution quotas.

Forked 2026-08-11 from `2_7_prediction_market` (identical engine and book contract)
so Candle Bet is its own published game; pairs with web `apps/2-10-candle-bet`.

## Modes: a three-tier candle-size ladder (cost 1.0), each one win/lose split

UP and DOWN are **symmetric** (identical odds), so from the book's point of view
the direction is cosmetic — the book encodes only win/lose and the offered
multiplier. The frontend derives the candle's direction from the player's chosen
side + `isWin` (`candleUp = pickedUp == isWin`) and its MAGNITUDE from the selected
size tier (the mode); the thresholds/target line are client rendering of the
certified verdict. Each published mode therefore covers both buttons; the bet
chips ($10/$50/$100/MAX in the UI) are ACP **bet levels**, not published modes.
...
books, so the published odds equal the book counts (optimiser off):
    ...
    ANY    2.00x  ->  p = 12/25 (~48.00%)  ->  RTP = 96.00%   (std 0.999)
    BIG    5.00x  ->  p =  5/26 (~19.23%)  ->  RTP = 96.15%   (std 1.971)
    HUGE  10.00x  ->  p =  5/52 (~ 9.62%)  ->  RTP = 96.15%   (std 2.948)

`wincap = 10.00` (the top tier's payout is the win cap), far under the ~100x
all-or-nothing ETL/CVaR ceiling.

**Volatility floor note:** the ACP two-outcome payout-std floor is ~0.60. The parent
game's 1.40x rung (std 0.649) was the lowest grid value clearing it; Candle Bet
deliberately drops that rung — no honest candle-size rule yields a 68.75% win
chance — so its lowest tier is 2.00x (std 0.999), well clear of the floor. Do not
add a rung below 1.40x without re-checking the volatility validators.

## ACP rules satisfied

  1. 0.1x LUT grid — every payout is a whole multiple of 10 cents; `lut_grid_exempt = False`
     keeps the check on.
  2. Per-mode RTP in [90%, 96.70%] (realised 96.00–96.15%).
  3. Cross-mode spread <= 1.00% — realised spread 0.15%.
"""
...
# Published payout multiplier(s) — one per candle-size tier. Each becomes its own
# dot-free mode "call_<cents>" with an independently-derived win probability that pins
# realised RTP into [96.00%, 96.70%]. One symmetric multiplier covers UP and DOWN.
# Size ladder: ANY 2.00x / BIG 5.00x / HUGE 10.00x (mode keys unchanged from the
# parent 2_7 game so the web app's tier mapping carries over). Every value is a
# multiple of 0.10 (0.1x LUT grid) and the highest (10.00x) stays well under the
# all-or-nothing tail cap; the lowest (2.00x, std 0.999) clears the ~0.60 ACP
# volatility floor with margin — see the docstring before adding lower rungs.
_MULTIPLIERS = [2.00, 5.00, 10.00]
...
class GameConfig(Config):
    """Candle Bet configuration — a three-tier candle-size ladder (RTP 96.00–96.70%)."""
    ...
    def _build_tiers(self) -> list:
        """One row per payout multiplier (the three candle-size tiers).
    ...
    def _build_bet_modes(self) -> list:
        """One bet mode ('base'); a forced win/lose split. UP and DOWN share it."""
        dummy_reels = {
            "reel_weights": {
