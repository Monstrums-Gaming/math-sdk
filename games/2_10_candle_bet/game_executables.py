"""Orchestrating routines for Candle Bet (2_10)."""
...
class GameExecutables(GameCalculations):
    """Resolve a single UP/DOWN call and emit its client events."""

    def evaluate_call(self) -> None:
        """Resolve the round for the active criteria and emit the outcome event.

        UP/DOWN are symmetric, so the round is direction-neutral: the criteria
