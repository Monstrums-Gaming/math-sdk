#!/usr/bin/env python3
"""
Build the frontend-demo data bundle for Dice Hard (2_4) from the REAL generated
...
and emits ONE compact file the demo fetches:
    frontend_demo/dice_hard_rgs.json

Because every dice lookup table has uniform weight 1 and at most two distinct
...
# the demo just checks each mode stays within it. Matches the ACP band enforced
# by game_config.py (RTP_CEIL 0.967); keep in sync with that constant.
...
OUT_FILE = os.path.join(HERE, "dice_hard_rgs.json")


def _read_lut(path):
    """Return (win_payout_cents, win_rows, loss_rows) for a dice lookup table.
...
def main():
    ...
    game_id = cfg.get("gameID", "2_4_dice_hard")
    ...
    print(f"  RTP range: {min(rtps)}..{max(rtps)} (ACP band {RTP_FLOOR}-{RTP_CEIL})")
