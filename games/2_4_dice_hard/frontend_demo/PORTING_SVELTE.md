# Dice Hard — RGS integration reference (for porting to Svelte)
...
All references point at `dice-hard.js` (the client) and `build_demo_data.py` (the bundle
...
`dice-hard.js` into a plain `.ts`/`.js` module; only the drawing code needs to become Svelte
components.
...
Mode is **auto-detected from the launch URL query params** (`dice-hard.js:39-46`):
...
- **LOCAL REPLAY** — anything else (e.g. opened directly / served locally). Outcomes are a weighted
  pick from the mode's real lookup table baked into `dice_hard_rgs.json`. Fully offline.

A badge (`setModeBadge`, `dice-hard.js:646`) shows which mode is active. If a LIVE
...
### Transport
All calls are `POST` + JSON to `https://${rgs_url}${endpoint}` (`rgsCall`, `dice-hard.js:205-216`).
...
(`dice-hard.js:377, 390-415`). On authenticate, an already-`active` round sets `hasOpenRound = true`.
...
"bet-level template"). `snapLiveBet()` (`dice-hard.js:227-238`) snaps a requested bet to the nearest
...
### Error codes
`rgsErrorMessage()` (`dice-hard.js:526-534`) maps RGS codes to user text:
...
`dice_hard_rgs.json` (`dice-hard.js:240-300`):
...
Every dice lookup table has **uniform weight 1** and exactly **two distinct payouts** — `0` (loss)
...
Books store **no** dice number. `syntheticRoll(mode, isWin)` (`dice-hard.js:307-312`) fabricates a
...
`playEvents(events)` (`dice-hard.js:314-339`) is the single replay path. It reads:

**Round fields** (used by `rollDice`, `dice-hard.js:379-396`):
...
Helpers (`dice-hard.js:26-28, 124-127`):
...
Display: `fmtMoney()` + the `CURRENCY_META` table (`dice-hard.js:49-63`) hold per-currency symbol
...
## 6. `dice_hard_rgs.json` — structure and role

This is the **offline odds bundle** the demo `fetch`es once at startup (`loadBundle`,
`dice-hard.js:653-674`; `DATA_URL = "./dice_hard_rgs.json"`).
...
- `modeName(direction, nn)` → `over_NN` / `under_NN` (`dice-hard.js:187-188`) — this string is the
  RGS `mode` argument.
...
- The ACP-compliant target set is **sparse** (gaps). `loadBundle` collects the sorted list of
  ...
  the nearest published target** (`dice-hard.js:190-197`). Your port must snap the same way — an
  arbitrary slider value has no mode.

### It is generated, not source
`dice_hard_rgs.json` is produced by **`build_demo_data.py`** from the game's published library:
...
```sh
env/bin/python games/2_4_dice_hard/frontend_demo/build_demo_data.py
```
...
**State**: the single `state` object (`dice-hard.js:100-122`) maps naturally onto a Svelte store or
`$state` runes. Derive `ranges` and sorted `targets` per direction exactly as `loadBundle` does.

**Data loading**: load `dice_hard_rgs.json` once at init — import it as a static asset or `fetch`
...
- Client: `games/2_4_dice_hard/frontend_demo/dice-hard.js`
- Bundle generator: `games/2_4_dice_hard/frontend_demo/build_demo_data.py`
- Demo overview: `games/2_4_dice_hard/frontend_demo/README.md`
