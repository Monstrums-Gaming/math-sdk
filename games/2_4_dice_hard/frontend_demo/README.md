# Dice Hard — frontend demo

A standalone, **zero-build** web frontend for `2_4_dice_hard` (a Stake-style dice game). It
...
frontend_demo/
  ...
  dice-hard.js          # RGS client + event replay (ES module)
  dice_hard_rgs.json    # GENERATED odds bundle (do not hand-edit)
...
- LOCAL mode reproduces the odds exactly because every dice lookup table has **uniform weight 1**
...
./games/2_4_dice_hard/frontend_demo/run.sh          # http://localhost:7810/index.html
```
...
`dice_hard_rgs.json` is **generated**, not source. Re-run `build_demo_data.py` after any math
