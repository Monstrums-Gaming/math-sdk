#!/usr/bin/env bash
# Serve the Crypto Pulse Grid frontend demo locally over HTTP.
# The demo fetch()es crypto_pulse_grid_rgs.json, so it must be served over http:// (file:// won't work).
# Usage:
...
echo "Crypto Pulse Grid demo → ${URL}"
