#!/usr/bin/env python3
"""
Build the frontend-demo data bundle for Price Grid (2_11) from the REAL generated
math output, so the browser demo replays the published lookup tables instead of
faking odds client-side.
...
fetches:
    frontend_demo/price_grid_rgs.json
...
OUT_FILE = os.path.join(HERE, "price_grid_rgs.json")
...
def main():
    ...
    assert bundle["game_id"] == "2_11_price_grid", f"unexpected game_id {bundle['game_id']}"
    modes = bundle["modes"]
