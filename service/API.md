...
```json
{
  ...
  "rtp": 0.89,
  "wincap": 200.0,
  "job": { "job_id": "…", "game_id": "3_lucky_dip", "mode": "prod", "status": "queued", "num_sims": 100000 }
}
```
...
**`BuildAccepted` (202)**
```json
{ "job_id": "a1b2c3d4e5f6…", "game_id": "3_2_cash_paradise_unit", "mode": "prod", "status": "queued", "num_sims": 100000 }
```
...
| `num_sims` | int \| null | Sims this build ran (`dev` forces `1000`; `prod` uses the manifest `build.num_sims`, default `100000`). |
...
```json
{
  ...
  "status": "succeeded", "num_sims": 100000, "publishable": true, "local_available": false,
  "error": null,
