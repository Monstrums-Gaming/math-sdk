...
| `2_4_dice_hard` | direct-prob win/lose | dice roll over/under | copy of `2_4_dice_kong_climb` — identical 192-mode NON-STAKE cent-grid ladder (RTP 97.18–98.0%, off the 0.1× grid, `lut_grid_exempt=True`, so NOT Stake-publishable as-is; docstring shows the Stake-legal revert). Here `game_id` == folder == `2_4_dice_hard` (no id≠folder legacy). No `frontend_demo/`. `stake-dice-game` skill |
...
| `2_7_prediction_market` | direct-prob win/lose | **Crypto Pulse** — HIGH/LOW binary, BTC chart ends above/below start line (internal `game_id` matches the folder) | **4 difficulty modes** `call_<cents>`, one per multiplier in `_MULTIPLIERS` (`game_config.py`): Easy 1.40× / Medium 1.90× / Hard 3.00× / Expert 5.00×. Each mode's win prob = `_simplest_fraction_in` pinning RTP into [96.0, 96.7]% (Medium `p=29/57`, `num_sims=57`; per-mode `num_sims = denominator`); grid-legal payouts (mult of 0.1×), cross-mode RTP spread ≤ 1%. HIGH/LOW symmetric ⇒ **direction-neutral book** (win/lose only); `priceCall`+`finalWin` events; `frontend_demo/` = animated BTC chart. Add/remove tiers by editing `_MULTIPLIERS` — everything regenerates |
| `2_6_tap_trade` | direct-prob win/lose | **Tap Trade** — **tap-cell-to-bet** (Euphoria mechanic): continuous chart, future region covered by a multiplier-cell grid; tap a cell to place a chip, line hits it ⇒ `bet×M`, else lose | a **2_7_prediction_market twin** (each chip = an independent win/lose bet at a fixed multiplier; the tapped cell is outcome-neutral like 2_9's HIGH/LOW). **28-rung ladder** 1.4–100×, **dense below 10×** (18 rungs) ⇒ modes `call_140`…`call_10000`, each a cost-1.0 win/lose `BetMode`, `max_win = its OWN M`. Win chance = `_simplest_fraction_in([0.960/M, 0.967/M])` (2_9 Stern-Brocot), `num_sims = denominator` (max 135; **1,176 books total**); realised RTP 96.00–96.55%, spread 0.5517%. **Floor is 1.4× not 1.2×** (a 1.2× win/lose mode has std 0.48 < the ACP 0.60 Base-Volatility floor); 100× cap = Limbo `base_100` precedent. Events `cellCall`(result "Win"/"Lose", isWin, payoutMultiplier int cents present win-or-lose, winChance) → base `finalWin`(amount only). Because `max_win = own M`, the per-mode wincap override (`run_sims.py` sets `config.wincap = BetMode.max_win`) makes a standard `wincap` event fire on **every winning book in every mode** (`cellCall → wincap → finalWin`; lose = `cellCall → finalWin`) — intentional, web treats it as a no-op. `build_odds_bundle.py` re-verifies publish artifacts + emits `odds_bundle.json` (kept OUT of `publish_files/`, consumed by the web app). `frontend_demo/` is a full canvas client: LOCAL mode replays the published LUT odds, LIVE mode places real RGS bets (`/wallet/play` at `call_<cents>`, then `/wallet/end-round` **only on a win** — a loss is already settled inside the play response; `round.payoutMultiplier` is a **plain** multiplier, not ×100 cents, unlike the nested `state[]` events), line **steered** to hit/miss the tapped cell per the drawn book, **max 2 chips per time-column on distinct cells** (a 3rd allows an unrenderable win-lose-win sandwich; cap is static so rejections leak nothing). `stake-price-grid-game` skill. (Lineage: `2_11_price_grid` → renamed 2026-07-20 to `2_11_crypto_pulse_grid`, taking over the `2_10_crypto_pulse_grid` slug after that earlier 20-rung build — paired with web `apps/2-1-crypto-pulse-grid` — was removed the same day → renamed 2026-07-22 to `2_6_tap_trade` to pair with web `apps/2-6-tap-trade` (in progress). Note the `2_6` number prefix is shared with `2_6_plinko` — folder/game_id stay unique.) |
...
Eight repo-local Claude skills encode the hard-won ACP/build knowledge — invoke them (or read their `SKILL.md`) instead of re-deriving the rules:
...
- **`stake-price-grid-game`** — the **tap-cell / Euphoria** family (`2_6_tap_trade`, formerly `2_11_crypto_pulse_grid`/`2_11_price_grid`; its retired predecessor `2_10_crypto_pulse_grid` followed the same recipe): the multiplier-ladder recipe (0.1× grid, 1.4× volatility floor, 100× cap, `_simplest_fraction_in` exact book counts), position-neutral books with one `call_<cents>` mode per rung, the intentional per-mode-wincap event quirk, the `cellCall → [wincap] → finalWin` contract, the build + `build_odds_bundle.py` pipeline, and the frontend rules (steered-line reveal, LOCAL LUT replay vs LIVE RGS bets, max 2 chips per time-column and why). Complements `stake-direct-probability-game`, `stake-risk-validators`, and `publish-stake-game`.
...
- **`stake-provably-fair`** — make a game provably-fair-verifiable + add fairness/odds transparency. The REAL model: the Stake **platform** owns the seed pair (client + hashed server + nonce); its `HMAC-SHA256 → float` **selects which pre-generated book** the RGS serves from a mode's weighted LUT (the game has **no** seed API — don't build an in-game seed panel). The game's job: store a **seeded, outcome-consistent result in the book** (dice = a `roll`, generated in integer-hundredths so replays are stable and win/lose ranges never flip) — an **event-structure change ⇒ rebuild + republish** (odds/LUT untouched) — plus the SHA-256 odds-integrity manifest (`fairness_manifest.py` → `fairness.json`) and web Fairness modal. Do NOT ship an in-browser live re-hash (404s in the live RGS; CSP blocks it). Reference: `games/2_4_dice_kong_climb` + web `apps/2-0-dice`. Complements `stake-dice-game` and `publish-stake-game`.

Together `stake-dice-game` + `stake-direct-probability-game` + `stake-risk-validators` cover the whole direct-probability game family (dice `2_4`, limbo `2_5`, plinko `2_6`, chicken `2_7`/`2_8`), with `stake-price-grid-game` adding the tap-cell grid branch (`2_6_tap_trade`); `publish-stake-game` owns the ACP upload for all of them, and `stake-provably-fair` adds seed-consistent results + fairness/odds transparency across the family.
