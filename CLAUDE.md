...
| `2_10_crypto_pulse_grid` | direct-prob win/lose | **Crypto Pulse Grid** — **tap-cell-to-bet** (Euphoria mechanic): continuous chart, future region covered by a multiplier-cell grid; tap a cell to place a chip, line hits it ⇒ `bet×M`, else lose | a **2_9_crypto_pulse twin** (each chip = an independent win/lose bet at a fixed multiplier; the tapped cell is outcome-neutral like 2_9's HIGH/LOW). **20-rung ladder** 1.4–100× ⇒ modes `call_140`…`call_10000`, each a cost-1.0 win/lose `BetMode`, `max_win = its OWN M`. Win chance = `_simplest_fraction_in([0.960/M, 0.967/M])` (2_9 Stern-Brocot), `num_sims = denominator` (max 135; 1,049 books total); realised RTP 96.00–96.39%, spread 0.3855%. **Floor is 1.4× not 1.2×** (a 1.2× win/lose mode has std 0.48 < the ACP 0.60 Base-Volatility floor); 100× cap = Limbo `base_100` precedent. Events `cellCall`(result "Win"/"Lose", isWin, payoutMultiplier int cents present win-or-lose, winChance) → base `finalWin`(amount only). Because `max_win = own M`, the per-mode wincap override (`run_sims.py` sets `config.wincap = BetMode.max_win`) makes a standard `wincap` event fire on **every winning book in every mode** (`cellCall → wincap → finalWin`; lose = `cellCall → finalWin`) — intentional, web treats it as a no-op. `build_odds_bundle.py` re-verifies publish artifacts + emits `odds_bundle.json` (kept OUT of `publish_files/`, consumed by the web app). Pairs with web `apps/2-1-crypto-pulse-grid`. (Superseded an earlier discarded "collect" multi-outcome build.) |
| `2_11_price_grid` | direct-prob win/lose | **Price Grid** — denser-ladder **2_10 variant** (same tap-cell/Euphoria mechanic, book contract and per-mode-wincap quirk) | **28-rung ladder** 1.4–100×, **dense below 10×** (18 rungs vs 2_10's 10: adds 1.5/1.6/1.8/2.2/2.8/3.2/3.6/4.5/7/9) ⇒ modes `call_140`…`call_10000`; same `_simplest_fraction_in` win chances, `num_sims = denominator` (max 135; **1,176 books total**); realised RTP 96.00–96.55%, spread 0.5517%; risk envelope identical to 2_10 at both ends (1.4× floor / 100× cap). `frontend_demo/` is a full canvas client: LOCAL mode replays the published LUT odds, LIVE mode places real RGS bets (`/wallet/play` at `call_<cents>` → `/wallet/end-round`), line **steered** to hit/miss the tapped cell per the drawn book, **max 2 chips per time-column on distinct cells** (a 3rd allows an unrenderable win-lose-win sandwich; cap is static so rejections leak nothing). `build_odds_bundle.py` → `odds_bundle.json`; `stake-price-grid-game` skill |
...
Seven repo-local Claude skills encode the hard-won ACP/build knowledge — invoke them (or read their `SKILL.md`) instead of re-deriving the rules:
...
- **`stake-price-grid-game`** — the **tap-cell / Euphoria** family (`2_10_crypto_pulse_grid`, `2_11_price_grid`): the multiplier-ladder recipe (0.1× grid, 1.4× volatility floor, 100× cap, `_simplest_fraction_in` exact book counts), position-neutral books with one `call_<cents>` mode per rung, the intentional per-mode-wincap event quirk, the `cellCall → [wincap] → finalWin` contract, the build + `build_odds_bundle.py` pipeline, and the frontend rules (steered-line reveal, LOCAL LUT replay vs LIVE RGS bets, max 2 chips per time-column and why). Complements `stake-direct-probability-game`, `stake-risk-validators`, and `publish-stake-game`.
...
Together `stake-dice-game` + `stake-direct-probability-game` + `stake-risk-validators` cover the whole direct-probability game family (dice `2_4`, limbo `2_5`, plinko `2_6`, chicken `2_7`/`2_8`), with `stake-price-grid-game` adding the tap-cell grid branch (`2_10`/`2_11`); `publish-stake-game` owns the ACP upload for all of them.
