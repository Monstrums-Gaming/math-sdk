...
- **`stake-price-grid-game`** — the **tap-cell / Euphoria** family (`2_11_crypto_pulse_grid`, formerly `2_11_price_grid`; its retired predecessor `2_10_crypto_pulse_grid` followed the same recipe): the multiplier-ladder recipe (0.1× grid, 1.4× volatility floor, 100× cap, `_simplest_fraction_in` exact book counts), position-neutral books with one `call_<cents>` mode per rung, the intentional per-mode-wincap event quirk, the `cellCall → [wincap] → finalWin` contract, the build + `build_odds_bundle.py` pipeline, and the frontend rules (steered-line reveal, LOCAL LUT replay vs LIVE RGS bets, max 2 chips per time-column and why). Complements `stake-direct-probability-game`, `stake-risk-validators`, and `publish-stake-game`.
...
Together `stake-dice-game` + `stake-direct-probability-game` + `stake-risk-validators` cover the whole direct-probability game family (dice `2_4`, limbo `2_5`, plinko `2_6`, chicken `2_7`/`2_8`), with `stake-price-grid-game` adding the tap-cell grid branch (`2_11`); `publish-stake-game` owns the ACP upload for all of them, and `stake-provably-fair` adds seed-consistent results + fairness/odds transparency across the family.
