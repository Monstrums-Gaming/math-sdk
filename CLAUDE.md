...
| `2_9_crypto_pulse` | direct-prob win/lose | **Crypto Pulse** — HIGH/LOW binary, BTC chart ends above/below start line (display name "Crypto Pulse"; internal `game_id` matches the folder, `2_9_crypto_pulse`) | single `base` mode (HIGH/LOW symmetric ⇒ **direction-neutral book**, only win/lose); mockup's 1.87× is off-grid so payout is **1.90×** (190¢), `p=29/57` via `_simplest_fraction_in`, `num_sims=57` (29 win/28 lose), RTP 96.67%; `priceCall`+`finalWin` events; `frontend_demo/` = animated BTC chart |
